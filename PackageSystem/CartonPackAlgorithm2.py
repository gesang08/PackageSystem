...
class Util:
    def calcPartWeightAreaVolume(self,part_lists):
        """
        计算部件重量和面积
    ...
    def calcPartSizeMax(self, part_lists):
        """
        获取该批部件的长度最大值和宽度最大值
        :param part_lists: 原始部件列表信息
            ...
            if len(box_lists) != 0:
                ...
                if len(box_temp_lists) != 0:
                    for box_temp_index, box_temp_item in enumerate(box_temp_lists):
                        min_distance = math.fabs(max_part_length - box_temp_lists[0][box_length_index]) + math.fabs(
                            max_part_width - box_temp_lists[0][box_width_index])  # 假设第一个元素为最小距离
                        distance = math.fabs(max_part_length - box_temp_item[box_length_index]) + math.fabs(
                            max_part_width - box_temp_item[box_width_index])
...
def PutPart(part_lists, box_lists, section_id):
    ...
    # while len(part_lists) != 0:
        # part_lists=util.convertSize(part_lists)
        ...
        # 测试最低水平线
    lowest = LowestHoriztonal(part_lists, box_lists)
...
class GA:
    ...
    def __init__(self, parts, boxes, chromosome_length, count, retain_rate, random_select_rate, mutation_rate, section_id):
        ...
        self.thick=self._parts[0][part_thickness_index]
    ...
    def initDecode(self):
        partList=self.util.partSort(copy.deepcopy(self._parts))
        initChromosome=[]
        for part in self._parts:
            partObj = PART()
            partObj.part_type_scheme = part[11]
            partObj.part_id_scheme = part[10]
            partObj.part_size_scheme = [part[4], part[5], part[6]]
            partObj.area_scheme = part[12]
            partObj.volume_scheme = part[13]
            partObj.weight_scheme = part[14]
            initChromosome.append(partObj)
    ...
    def sloution(self):
        ...
        fitness_graded = [(chromosome, self.fitness(chromosome)) for chromosome in self.population]
        fitness_graded = [x[0] for x in sorted(fitness_graded, key=lambda g: g[1], reverse=True)]
        return fitness_graded[0], self.decode(fitness_graded[0], convert_digit=True), self.fitness(fitness_graded[0])
...
class PART:
    ...
    def __init__(self):
        ...
        self.select_state=0#0:部件没有被选中，1：部件被选中
...
def CartonPackAlgorithmMain():
    ...
    if sec_id:
        ...
        if part_lists and box_lists:
            part_lists=util.calcPartWeightAreaVolume(copy.deepcopy(part_lists))
            part_lists=util.convertSize(part_lists)
            maxL,maxW=util.calcPartSizeMax(copy.deepcopy(part_lists))
            box = util.selectBoxByManhattanDistance(maxL,maxW,copy.deepcopy(box_lists))

            ga = GA(part_lists, box, len(part_lists), count=100, retain_rate=0.25, random_select_rate=0.5,
